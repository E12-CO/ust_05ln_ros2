...
class ust_05ln_if : public rclcpp::Node{
	...
	// Laser properties
	typedef struct{
		long unsigned int scan_length;
		long unsigned int scan_count;
		float range_max;
	...
	void hokuyo_fsm(){
		...
		switch(laser_fsm){
			...
			case 2:// PD query and ID probing
			{
				...
				find_ident = std::string(scan_buffer);
